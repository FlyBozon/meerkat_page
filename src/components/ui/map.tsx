...
type MapProps = {
  ...
  /** Explicit map style override. Takes precedence over theme-based styles. */
...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  const resolvedStyle = mapStyle ?? (resolvedTheme === "dark" ? mapStyles.dark : mapStyles.light);
  ...
  useEffect(() => {
    ...
    if (currentStyleRef.current === resolvedStyle) return;

    clearStyleTimeout();
    currentStyleRef.current = resolvedStyle;
    setIsStyleLoaded(false);

    mapInstance.setStyle(resolvedStyle, { diff: true });
  }, [mapInstance, resolvedStyle, clearStyleTimeout]);
